...
const PageLinks = ({ pagesNumber = 0, characterName, currentPage }: { pagesNumber: number, characterName: string | null, currentPage: number }) => {
        ...
        <ul className={classes.pageLinksList}>
            {
                visiblePages.map((page) => (
                    <li key={page}>
                        <Link className={`${classes.pageLinksLink} ${(currentPage === page) ? classes.active : ''}`} href={`?name=${characterName || ''}&page=${page}`}>
                            {page}
                        </Link>
                    </li>
                ))
